...
class ChatAI {
    ...
    constructor(options) {
        let defaults = {
            ...
            source: 'openai',
            model: 'gpt-3.5-turbo',
            backend: 'http://0.0.0.0:8012',
            endpoint: '/v1.0/text/ask',
            conversations: [],
    ...
    getMessage() {
        ...
        let messages = [...this.selectedConversation.messages].map(message => { 
            return { role: message.role, content: message.content } 
        });
        console.debug("fetching:"+this.options.backend+" endpoint "+ this.options.endpoint);
        console.debug(messages)
        fetch(this.options.backend+this.options.endpoint, { 
            //cache: 'no-cache',
            method: 'POST',
            headers: {
